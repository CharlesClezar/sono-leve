-- Marcas
INSERT INTO "Marca" ("Id", "Name", "Active")
SELECT gen_random_uuid(), v."Name", true
FROM (VALUES ('Sono Leve'), ('Clelia Anastácio'), ('Thainá Reichen'), ('Ronca&Fuça')) AS v("Name")
WHERE NOT EXISTS (SELECT 1 FROM "Marca" WHERE "Name" = v."Name");

-- Categorias
INSERT INTO "Categoria" ("Id", "Name", "Grade", "Active")
...
WHERE NOT EXISTS (SELECT 1 FROM "Categoria" WHERE "Name" = v."Name");

-- Tipos
INSERT INTO "Tipo" ("Id", "Name", "Active")
SELECT gen_random_uuid(), v."Name", true
FROM (VALUES ('Camisola'), ('Conjunto'), ('Macacão'), ('Pantufa'), ('Pescador')) AS v("Name")
WHERE NOT EXISTS (SELECT 1 FROM "Tipo" WHERE "Name" = v."Name");

-- Subtipos
INSERT INTO "Subtipo" ("Id", "Name", "Active")
SELECT gen_random_uuid(), v."Name", true
FROM (VALUES ('Alça'), ('Regata'), ('Manga Curta'), ('Manga Longa')) AS v("Name")
WHERE NOT EXISTS (SELECT 1 FROM "Subtipo" WHERE "Name" = v."Name");

-- Coleções
INSERT INTO "Colecao" ("Id", "Name", "DataInicio", "DataFim", "Active")
...
WHERE NOT EXISTS (SELECT 1 FROM "Colecao" WHERE "Name" = v."Name");

-- Formas de pagamento base (sem maquininha/link/tap — esses têm taxas configuradas abaixo)
INSERT INTO "FormaPagamento" ("Id", "Nome", "Tipo", "PermiteParcelamento", "ExigeBandeira", "RepassaTaxaAoCliente", "Ativo", "CriadoEm", "AtualizadoEm")
...
WHERE NOT EXISTS (SELECT 1 FROM "FormaPagamento" WHERE "Nome" = v."Nome");

-- Bandeiras de cartão
INSERT INTO "BandeiraCartao" ("Id", "Nome", "Ativo", "CriadoEm", "AtualizadoEm")
SELECT gen_random_uuid(), v."Nome", true, NOW(), NOW()
FROM (VALUES ('Visa'), ('Mastercard'), ('Elo'), ('Hipercard'), ('American Express')) AS v("Nome")
WHERE NOT EXISTS (SELECT 1 FROM "BandeiraCartao" WHERE "Nome" = v."Nome");

-- Cliente padrão para varejo sem identificação
INSERT INTO "Cliente" ("Id", "Nome", "Telefone", "Cpf", "Tipo", "Status", "Credito", "CriadoEm", "AtualizadoEm")
SELECT gen_random_uuid(), 'CONSUMIDOR FINAL', '', '', 0, 'Ativo', 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM "Cliente" WHERE "Nome" = 'CONSUMIDOR FINAL');

-- ─── Configurações de Taxa por Bandeira ───────────────────────────────────────
-- Tabela: ConfiguracaoTaxaCartao (FormaPagamentoId, BandeiraId, TipoCartao)
-- Tabela: ConfiguracaoTaxaCartaoParcela (ConfiguracaoTaxaCartaoId, NumeroParcelas, PercentualTaxa, PrazoRecebimentoDias)

INSERT INTO "ConfiguracaoTaxaCartao" ("Id", "FormaPagamentoId", "BandeiraId", "TipoCartao", "Ativo", "CriadoEm", "AtualizadoEm")
SELECT gen_random_uuid(),
       (SELECT "Id" FROM "FormaPagamento" WHERE "Nome" = v."FormaNome" LIMIT 1),
       (SELECT "Id" FROM "BandeiraCartao"  WHERE "Nome" = v."BandeiraNome" LIMIT 1),
       v."TipoCartao",
       true,
       NOW(),
       NOW()
...
WHERE NOT EXISTS (
  SELECT 1
  FROM "ConfiguracaoTaxaCartao" c
  JOIN "FormaPagamento" fp ON c."FormaPagamentoId" = fp."Id"
  JOIN "BandeiraCartao"  b  ON c."BandeiraId"      = b."Id"
...
INSERT INTO "ConfiguracaoTaxaCartaoParcela" (
...
WHERE NOT EXISTS (
  SELECT 1
  FROM "ConfiguracaoTaxaCartaoParcela" p
  JOIN "ConfiguracaoTaxaCartao" c ON p."ConfiguracaoTaxaCartaoId" = c."Id"
  JOIN "FormaPagamento" fp ON c."FormaPagamentoId" = fp."Id"
  JOIN "BandeiraCartao"  b  ON c."BandeiraId"      = b."Id"
  WHERE fp."Nome" = v."FormaNome"
    AND b."Nome"  = v."BandeiraNome"
