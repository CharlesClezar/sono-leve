...
log() { echo "[$(TZ="America/Sao_Paulo" date '+%Y-%m-%d %H:%M:%S')] $1" | tee -a "${LOG_FILE}"; }
...
if [ -n "${PARA_REMOVER}" ]; then
  REMOVED=$(echo "${PARA_REMOVER}" | wc -l | tr -d ' ')
  echo "${PARA_REMOVER}" | xargs rm -f
  log "→  ${REMOVED} backup(s) antigo(s) removido(s)"
fi

COUNT=$(ls "${BACKUP_DIR}"/*.sql.gz 2>/dev/null | wc -l | tr -d ' ')
log "✓  Total mantido: ${COUNT}/${RETENTION_MAX}"
